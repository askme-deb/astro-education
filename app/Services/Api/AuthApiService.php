...
class AuthApiService extends BaseApiClient implements Contracts\AuthApiServiceInterface
{
     ...
     * @return array<string, mixed>
     */
    protected function normalizeAuthResponse(array $response): array
    {
        ...
        $innerData = $payload['data'] ?? $payload;
        ...
        return [
            'success' => (bool) ($payload['success'] ?? $response['success'] ?? false),
            'status' => (int) ($response['status'] ?? 500),
            'message' => $payload['message'] ?? $response['message'] ?? null,
            'data' => $innerData,
            'user' => $innerData['user'] ?? $innerData['customer'] ?? null,
            'token' => $innerData['token'] ?? $innerData['access_token'] ?? null,
            'access_token' => $innerData['access_token'] ?? $innerData['token'] ?? null,
